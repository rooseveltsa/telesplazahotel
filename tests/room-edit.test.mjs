...
test('edição de quarto preserva reservas, valida capacidade e isola unidades',async()=>{
 ...
 const payload={action:'room_update',hotel:'unit',id:'room',number:'101A',category:'Triplo',capacity:3,rate:290,floor:'2',extra_guest_rate:50};
 ...
 const room=sqlite.prepare("SELECT * FROM rooms WHERE id='room'").get();assert.equal(room.number,'101A');assert.equal(room.rate,29000);assert.equal(room.state,'dirty');assert.equal(room.floor,'2');assert.equal(room.capacity,3);assert.equal(room.extra_guest_rate,5000);const {roomRate}=await import(pathToFileURL(dir+'/hotel-domain.mjs'));assert.equal(roomRate(room,2),29000);assert.equal(roomRate(room,3),34000);assert.equal(roomRate({rate:28000,extra_guest_rate:5000},3),33000);
